...
begin
  ...
  after_answer := public.submit_game_choice((fresh->>'run_id')::uuid, (scenario->>'id')::integer, wrong_index);
  ...
  retried := public.submit_game_choice((fresh->>'run_id')::uuid, (scenario->>'id')::integer, wrong_index);
  if retried->'results' <> after_answer->'results' or retried->>'balance' <> after_answer->>'balance' then raise exception 'Retry charged twice'; end if;
  ...
  begin
    perform public.submit_game_choice((fresh->>'run_id')::uuid, (scenario->>'id')::integer, wrong_index);
    raise exception 'Old run accepted';
  exception when invalid_parameter_value then null; end;
  begin
    perform public.submit_game_choice((after_reset->>'run_id')::uuid, (scenario->>'id')::integer, 99);
    raise exception 'Invalid answer accepted';
  exception when invalid_parameter_value then null; end;
end $$;
reset role;
select 'PASS: server scoring, atomic balance, duplicate retry, restricted writes, preserved history, idempotent reset, stale run, invalid answer' as tests;
