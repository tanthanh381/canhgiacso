...
type LogoTarget='logo'|'hdbankLogo';
export function CertificateEditor({template,onChange}:{template:CertificateTemplate;onChange:(value:CertificateTemplate)=>void}) {
 ...
 const uploadVersion=useRef<Record<LogoTarget,number>>({logo:0,hdbankLogo:0});
 const current=useRef(template);current.current=template;
 const active=design.elements[selected];
 const imageSelected=selected==='logo'||selected==='hdbankLogo';
 ...
 async function upload(file:File|undefined,target:LogoTarget){if(!file)return;const version=++uploadVersion.current[target];setError('');
  ...
  try{const bitmap=await createImageBitmap(file);const canvas=document.createElement('canvas');const scale=Math.min(1,600/Math.max(bitmap.width,bitmap.height));canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));canvas.getContext('2d')!.drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close();const data=canvas.toDataURL('image/webp',.85);if(data.length>400000)throw new Error();if(version!==uploadVersion.current[target])return;const base=current.current.design??defaultCertificateDesign();onChange({...current.current,design:{...base,[target]:data}});setSelected(target);}catch{setError('Không đọc được ảnh hoặc ảnh quá lớn. Hãy chọn logo nhỏ hơn.');}}
 function clearLogo(target:LogoTarget){uploadVersion.current[target]++;const base=current.current.design??defaultCertificateDesign();onChange({...current.current,design:{...base,[target]:''}});setSelected(target);}
 return <section className="cert-editor" aria-label="Trình thiết kế chứng chỉ">
  ...
  <button className="admin-secondary" onClick={()=>{uploadVersion.current.logo++;uploadVersion.current.hdbankLogo++;const {design:removed,...rest}=template;void removed;onChange(rest);}}>Về bố cục gốc</button></div>
  ...
  <div className="cert-workspace"><div className="cert-stage" ref={stage}>
   ...
   <fieldset><legend>Logo chứng chỉ</legend>
    <label>Logo Team/đơn vị (bên trái)<input type="file" accept="image/png,image/jpeg,image/webp" onChange={event=>{void upload(event.target.files?.[0],'logo');event.target.value='';}} /></label>
    {design.logo&&<button type="button" onClick={()=>clearLogo('logo')}>Xóa logo Team/đơn vị</button>}
    <label>Logo HDBank (bên phải)<input type="file" accept="image/png,image/jpeg,image/webp" onChange={event=>{void upload(event.target.files?.[0],'hdbankLogo');event.target.value='';}} /></label>
    {design.hdbankLogo&&<button type="button" onClick={()=>clearLogo('hdbankLogo')}>Xóa logo HDBank tùy chỉnh</button>}
    <small>Nếu chưa tải logo HDBank riêng, hệ thống dùng wordmark HDBank mặc định. Có thể chọn “Logo HDBank” ở mục Thành phần để kéo, đổi vị trí và kích thước.</small>
   </fieldset>
   ...
   <p>Dữ liệu trên mẫu chỉ để xem trước. Chọn “Lưu bản nháp” hoặc “Xuất bản” để lưu thiết kế và hai logo cùng nội dung.</p>
  </aside></div>
 </section>;
}
